...
static cli_cmd_t cmd_list[CLI_CMD_LIST_MAX];
...
//static uint8_t esc_state = 0; //방향키(세 가지 조합)
...
{
    if(cli_cmd_count>=CLI_CMD_LIST_MAX) return false;

    strncpy(cmd_list[cli_cmd_count].cmd_str, cmd_str, strlen(cmd_str));
    cmd_list[cli_cmd_count].cmd_func = cmd_func;
    cli_cmd_count++;
...
void cliRunCommand()
{
    ...
    bool is_found=false;
    for(uint8_t i=0; i<cli_cmd_count; i++){
        if(strcmp(cli_argv[0], cmd_list[i].cmd_str)==0){
            is_found=true;
            cmd_list[i].cmd_func(cli_argc, cli_argv);
            break;
        }
    }
